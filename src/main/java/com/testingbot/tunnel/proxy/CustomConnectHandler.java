...
public class CustomConnectHandler extends ConnectHandler {
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (method.equalsIgnoreCase("CONNECT")) {
            Logger.getLogger(CustomConnectHandler.class.getName()).log(Level.INFO, "<< [{0}] {1} ({2})",
                    new Object[]{method, request.getRequestURL().toString(), response.toString().substring(9, 12)});
        }
