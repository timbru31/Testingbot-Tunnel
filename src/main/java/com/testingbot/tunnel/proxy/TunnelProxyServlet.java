    ...
    protected void handleOnException(Throwable ex, HttpServletRequest request, HttpServletResponse response) {
        if (request.getRequestURL().toString().indexOf("squid-internal") == -1) {
            Logger.getLogger(TunnelProxyServlet.class.getName()).log(Level.WARNING, "{0} for request {1}\n{2}",
                    new Object[]{ex.getMessage(), request.getMethod() + " - " + request.getRequestURL().toString(), ExceptionUtils.getStackTrace(ex)});
        }
            ...
            if (!continuation.isInitial()) {
                ...
                CustomHttpExchange exchange = new CustomHttpExchange() {
                    ...
                    protected void onResponseComplete() throws IOException {
                        ...
                        Logger.getLogger(TunnelProxyServlet.class.getName()).log(Level.INFO, "<< [{0}] {1} ({2}) - {3} | {4}", new Object[]{request.getMethod(), request.getRequestURL().toString(),
                                response.toString().substring(9, 12), (endTime - this.startTime) + " ms", requestSize + " bytes"});
